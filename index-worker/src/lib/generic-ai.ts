...
function wrapModel(model: LMV2): LMV2 {
  ...
  return {
    ...
    doGenerate: async (options: any) => {
      // AI SDK 传 type:"json"+schema，OpenAI 驱动内部转 json_schema。
      // 免费模型不支持 json_schema，去 schema 退回无约束 JSON。
      if (options.responseFormat?.type === "json" && options.responseFormat?.schema) {
        const { schema, ...rest } = options.responseFormat;
        return model.doGenerate({ ...options, responseFormat: rest });
      }
